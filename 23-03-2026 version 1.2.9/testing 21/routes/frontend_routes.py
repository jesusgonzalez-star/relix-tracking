...
from flask import Blueprint, render_template, request, redirect, url_for, flash, session, send_file, send_from_directory, current_app, jsonify, abort
import logging
...
_STORAGE_STATE_MAP = {
    ...
    'EN BODEGA': 'EN_BODEGA',
    'EN_BODEGA': 'EN_BODEGA',
    'DISPONIBLE EN BODEGA': 'DISPONIBLE EN BODEGA',
    'EN RUTA': 'En Ruta',
    'ENTREGADO': 'Entregado',
    'PENDIENTE EN SOFTLAND': 'PENDIENTE_EN_SOFTLAND',
    'PENDIENTE_EN_SOFTLAND': 'PENDIENTE_EN_SOFTLAND',
}
...
def _normalize_cc_assignments(raw_value):
...
def _build_softland_cc_match_clause(alias: str, token_count: int):
    """
    Devuelve cláusula SQL para matchear CC contra descripción y código.
    Evita el falso negativo de COALESCE(DescCC, CodiCC) cuando el usuario tiene
    asignado el código pero en la OC viene con descripción (o viceversa).
    """
    if token_count <= 0:
        return "1=0", []
    ph = ",".join(["?"] * token_count)
...
# Idéntico al AS CentroCosto del listado bodega (Desc prioritaria; si no hay, código; si no hay, 'Sin CC').
_SOFTLAND_OC_CC_DISPLAY_AS_IN_TABLE = """COALESCE(
...
def _append_softland_oc_cc_dashboard_clause(where_parts, where_params, cc_filter_raw):
    """Solo OCs cuyo CC mostrado coincide exactamente con la opción elegida (no por palabras sueltas)."""
    raw = " ".join(((cc_filter_raw or "").strip()).split())
    if not raw:
        return
    where_parts.append(
        f"(LTRIM(RTRIM({_SOFTLAND_OC_CC_DISPLAY_AS_IN_TABLE})) COLLATE Latin1_General_CI_AI = "
        f"LTRIM(RTRIM(?)) COLLATE Latin1_General_CI_AI)"
    )
    where_params.append(raw)
...
def _normalize_cc_dashboard_label(s: str) -> str:
    """Etiqueta comparable al combo / celda (espacios + mayúsculas + sin tildes)."""
    return _strip_accents_upper(" ".join((s or "").split()))


def _orden_row_matches_cc_dashboard(orden, cc_filter_raw: str) -> bool:
    if not (cc_filter_raw or "").strip():
        return True
    cc_txt = str(orden[13] if len(orden) > 13 else "") or ""
    cc_u = _normalize_cc_dashboard_label(cc_txt)
    if not cc_u or cc_u == "SIN CC":
        return False
    return _normalize_cc_dashboard_label(cc_filter_raw) == cc_u


def _filter_folio_ints_by_softland_cc_dashboard(cursor_s, folios, cc_filter_raw):
    """Filtra folios ya conocidos (tracking local, listas auxiliares) por CC en Softland."""
    raw = " ".join(((cc_filter_raw or "").strip()).split())
    if not raw or not folios:
        return list(folios)
    ids_order = []
    seen_ids = set()
    for f in folios:
    ...
    allowed = set()
    cap = 500
    for i in range(0, len(ids_order), cap):
        chunk = ids_order[i : i + cap]
        ph = ",".join(["?"] * len(chunk))
        sql = f"""
            SELECT OC.NumOc
            FROM softland.OW_vsnpTraeEncabezadoOCompra OC WITH (NOLOCK)
            WHERE OC.NumOc IN ({ph})
              AND {cc_clause}
        """
        cursor_s.execute(sql, tuple(chunk + [raw]))
...
def _get_faena_cc_assignments(cursor_local, user_id):
    _ensure_faena_cc_column(cursor_local)
    cursor_local.execute("SELECT CentrosCostoAsignados FROM UsuariosSistema WHERE Id = ?", (user_id,))
    row = cursor_local.fetchone()
    return _normalize_cc_assignments(row[0] if row else "")


def _get_folios_by_centros_costo(cc_tokens):
    if not cc_tokens:
        return set()
    folios = set()
    try:
        ...
        import pyodbc
        conn_cc = pyodbc.connect(SoftlandConfig.get_connection_string(), timeout=10)
        try:
            c = conn_cc.cursor()
            cc_match_sql, _ = _build_softland_cc_match_clause('OC', len(cc_tokens))
            c.execute(
                f"""
                SELECT OC.NumOc
                FROM softland.OW_vsnpTraeEncabezadoOCompra OC WITH (NOLOCK)
...
def _get_folios_softland_oc_cc_display_exact(cc_filter_raw):
    """
    OCs cuya cabecera CC coincide con el texto del combo (misma expresión que el listado bodega).
    """
    raw = " ".join(((cc_filter_raw or "").strip()).split())
    if not raw:
        return set()
    try:
        ...
        conn_cc = pyodbc.connect(SoftlandConfig.get_connection_string(), timeout=15)
        try:
            c = conn_cc.cursor()
            c.execute(
                f"""
                SELECT OC.NumOc
                FROM softland.OW_vsnpTraeEncabezadoOCompra OC WITH (NOLOCK)
                WHERE LTRIM(RTRIM({_SOFTLAND_OC_CC_DISPLAY_AS_IN_TABLE})) COLLATE Latin1_General_CI_AI
                    = LTRIM(RTRIM(?)) COLLATE Latin1_General_CI_AI
                """,
                (raw,),
            )
            return {int(r[0]) for r in c.fetchall() if r and r[0] is not None}
        finally:
            conn_cc.close()
    except Exception as exc:
        logger.warning("No fue posible listar folios por CC exacto (faena/bodega): %s", exc)
        return set()


def _folio_matches_centros_costo_tokens(folio, cc_tokens):
...
def _faena_user_has_cc_access_to_folio(user_id, folio):
    conn_u = DatabaseConnection.get_connection()
    if not conn_u:
        return False
    try:
        cu = conn_u.cursor()
        cc_tokens = _get_faena_cc_assignments(cu, user_id)
    finally:
        conn_u.close()
    if not cc_tokens:
        return False
    return _folio_matches_centros_costo_tokens(folio, cc_tokens)


def _faena_softland_items_match_assigned_cc(despacho_items, faena_cc_tokens):
    """
    True si alguna línea de la OC en Softland (centro_costo_linea / partida) coincide con los CC del FAENA.
    Cubre cabeceras ERP que no calzan pero sí las líneas de owordendet / requisición.
    """
    if not faena_cc_tokens or not despacho_items:
        return False
    tokens = {(t or '').strip().upper() for t in faena_cc_tokens if (t or '').strip()}
    if not tokens:
        return False
    for item in despacho_items:
        if not isinstance(item, dict):
            continue
        cc_raw = item.get('centro_costo_linea')
        cc_norm = ' '.join(str(cc_raw or '').upper().split())
        if not cc_norm or cc_norm == 'SIN CC':
            continue
        parts = [p for p in re.split(r'[\s\-;/]+', cc_norm) if p]
        for t in tokens:
            if len(t) < 2:
                continue
            if t in cc_norm or t in parts:
                return True
    return False


def _form_cc_assignments_from_request():
...
def _fetch_softland_centros_costo_opciones(usuarios_rows=None):
    """
    Lista ordenada de CC para desplegables (misma lógica que filtros vs OC en Softland).
    Incluye CC ya asignados a usuarios FAENA aunque ya no aparezcan en el ERP.
    """
    opciones = []
    try:
        from config import SoftlandConfig
        import pyodbc
        conn = pyodbc.connect(SoftlandConfig.get_connection_string(), timeout=15)
        try:
            c = conn.cursor()
            c.execute(
                """
    ...
    if usuarios_rows:
        for u in usuarios_rows:
            if len(u) > 6 and (u[4] or '').upper() == 'FAENA' and u[6]:
                for t in _normalize_cc_assignments(u[6]):
                    if t not in seen:
                        seen.add(t)
                        opciones.append(t)
    opciones.sort()
...
def _dashboard_centros_costo_opciones(ordenes):
    """Lista de CC para el filtro del panel: mismos distinct ERP que superadmin + CC presentes en la página."""
    base = _fetch_softland_centros_costo_opciones(None)
    seen = set(base)
    for ord in ordenes or []:
        if len(ord) > 13 and ord[13]:
            for t in _normalize_cc_assignments(str(ord[13])):
                if t not in seen:
                    seen.add(t)
                    base.append(t)
    base.sort()
...
def _dashboard_centros_costo_opciones_faena(ordenes, cc_asignados):
    allowed = set(_normalize_cc_assignments(", ".join(cc_asignados or [])))
    for ord in ordenes or []:
        if len(ord) > 13 and ord[13]:
            allowed.update(_normalize_cc_assignments(str(ord[13])))
    opciones = sorted([x for x in allowed if x and x not in ('SIN CC',)])
    return opciones
...
def _ensure_faena_cc_column(cursor):
...
def _sum_enviado_por_linea(cursor, num_oc):
    """Suma CantidadEnviada por (NumLineaOc, CodProd) en todos los envíos no anulados."""
    cursor.execute("""
        SELECT
            D.NumLineaOc,
            LTRIM(RTRIM(COALESCE(D.CodProd, ''))) AS CodProd,
    ...
    for row in cursor.fetchall():
        ...
        key = (linea, cod)
        sums[key] = sums.get(key, 0.0) + float(row[2] or 0)
    return sums


def _sum_enviado_por_numlinea(cursor, num_oc):
    """Suma CantidadEnviada por NumLineaOc (todas las guías no anuladas). Evita bloqueos si CodProd no coincide con Softland."""
    cursor.execute(
        """
        SELECT
            D.NumLineaOc,
...
def _compute_entrega_parcial_bodega_envio(despacho_items_softland, sent_map_before, selected_items, sent_map_line=None):
    ...
    def _prev_sent(num_linea, codprod):
        ...
        if sent_map_line is not None and linea is not None:
            return float(sent_map_line.get(linea, 0.0))
        k = _bodega_envio_line_key(num_linea, codprod)
        return float(sent_map_before.get(k, 0.0))
    ...
    for it in despacho_items_softland or []:
        try:
            qty_in = float(it.get('qty_ingresada') or 0)
        except Exception:
            qty_in = 0.0
        if qty_in <= 1e-9:
            continue
        ...
        pend = max(qty_in - prev, 0.0)
        if pend <= 1e-9:
            continue
        if k not in selected_keys:
            return True
    for item in selected_items or []:
        k = _bodega_envio_line_key(item[1], item[2])
        try:
            qty_in = float(item[5] or 0)
            prev = _prev_sent(item[1], item[2])
            pend = max(qty_in - prev, 0.0)
            qty_send = float(item[6] or 0)
            qty_sol = float(item[4] or 0)
        except Exception:
            continue
        if qty_send + 1e-9 < pend or qty_send + 1e-9 < qty_sol:
            return True
    return False
...
def _oc_has_pending_bodega_dispatch(cursor, num_oc, despacho_items_softland):
    ...
    for item in despacho_items_softland:
        try:
            qty_in = float(item.get('qty_ingresada') or 0)
        except Exception:
            qty_in = 0.0
        if qty_in <= 0:
            continue
        linea = _normalize_oc_linea_num(item.get('num_linea'))
        already = float(sent_line.get(linea, 0.0)) if linea is not None else 0.0
        if qty_in - already > 1e-6:
            return True
    return False
...
def _oc_has_pending_warehouse_reception(despacho_items_softland):
    ...
    for item in despacho_items_softland or []:
        try:
            qty_in = float(item.get('qty_ingresada') or 0)
            qty_sol = float(item.get('qty_solicitada') or 0)
        except Exception:
            continue
        if qty_sol <= 1e-9:
            continue
        if qty_in + 1e-9 < qty_sol:
            return True
    return False
...
def _sync_despachos_tracking_header(cursor, conn, num_oc, despacho_items_softland=None):
    ...
    cursor.execute("SELECT TOP 1 Id FROM DespachosTracking WHERE NumOc = ? ORDER BY Id DESC", (num_oc,))
    trk = cursor.fetchone()
    if not trk:
        return
    ...
    if en_ruta:
        cursor.execute("""
            ...
            WHERE Id = ?
        """, (en_ruta[2], en_ruta[3], en_ruta[5], en_ruta[6], en_ruta[7], en_ruta[8], trk_id))
        conn.commit()
        return
    ...
    if pending_dispatch or pending_reception:
        cursor.execute("""
            UPDATE DespachosTracking SET
                Estado = 'EN_BODEGA',
                GuiaDespacho = NULL,
        ...
        cursor.execute("SELECT COUNT(1) FROM DespachosEnvio WHERE NumOc = ?", (num_oc,))
        if int(cursor.fetchone()[0] or 0) == 0:
...
def _ensure_local_tracking_table(cursor, conn=None):
    ...
    """)
    cursor.execute("""
        IF COL_LENGTH('DespachosEnvioDetalle', 'CantidadRecibidaFaena') IS NULL
            ALTER TABLE DespachosEnvioDetalle ADD CantidadRecibidaFaena DECIMAL(18,4) NULL;
    """)
    cursor.execute("""
...
def _is_valid_patente(value):
    """Valida patente evitando entradas basura y aceptando formatos operativos comunes."""
    pat = _normalize_patente(value)
    if not pat:
        return False
    # Largo razonable y caracteres permitidos.
    if re.match(r"^[A-Z0-9-]{5,8}$", pat) is None:
        return False
    # Debe contener al menos una letra y al menos un número.
    if not re.search(r"[A-Z]", pat):
        return False
    if not re.search(r"[0-9]", pat):
        return False
    # No permitir solo guiones o guiones consecutivos excesivos.
    if re.match(r"^-+$", pat):
        return False
    if "--" in pat:
        return False
...
def _extract_folio_from_evidence_filename(filename):
    """Intenta obtener folio desde nombres tipo despacho_<folio>_* o entrega_<folio>_*."""
    name = (filename or "").strip()
    if not name:
        return None
    m = re.match(r"^(?:despacho|entrega)_(\d+)_", name, flags=re.IGNORECASE)
    if not m:
        return None
    try:
...
def _pendiente_bodega(ing, sent):
    """Resta ingresado − enviado con Decimal para evitar residuos float falsos."""
    ing_d = Decimal(str(ing or 0))
    sent_d = Decimal(str(sent or 0))
    p = ing_d - sent_d
    if p < 0:
        p = Decimal('0')
...
def _softland_oc_qty_solicitada_total_map(folios):
    ...
    out = {}
    try:
        conn = pyodbc.connect(SoftlandConfig.get_connection_string(), timeout=10)
        try:
            cur = conn.cursor()
    ...
    cursor.execute(
        ...
        ;WITH L AS (
            SELECT
                ...
                SUM(CASE WHEN UPPER(LTRIM(RTRIM(ISNULL(D.EstadoLinea, '')))) = 'ENTREGADO'
                    THEN COALESCE(
                        TRY_CONVERT(DECIMAL(18,4), D.CantidadRecibidaFaena),
                        TRY_CONVERT(DECIMAL(18,4), D.CantidadEnviada)
...
def _parse_qty_faena_recepcion_linea(raw, max_qty):
    """Cantidad declarada en faena: 0..max (enviada en la guía). None = inválido."""
    if raw is None:
        return None
    s = str(raw).strip()
    if s == '':
    ...
    if q > mx + 1e-5:
        return None
    return max(0.0, min(q, mx))


def _map_cantidad_recibida_faena_por_linea_oc(cursor, num_oc):
    """
    Cantidad ya recepcionada en faena por línea de OC: suma COALESCE(CantidadRecibidaFaena, CantidadEnviada)
    donde EstadoLinea = ENTREGADO. Incluye legacy DespachosTrackingDetalle solo si la OC no tiene filas en DespachosEnvioDetalle.
    """
    out = {}
    num_oc = int(num_oc)
    try:
        cursor.execute(
            ...
            SELECT
                ...
                SUM(CAST(COALESCE(D.CantidadRecibidaFaena, D.CantidadEnviada) AS DECIMAL(18, 4))) AS Q
            FROM DespachosEnvioDetalle D
            WHERE D.NumOc = ?
              AND UPPER(LTRIM(RTRIM(D.EstadoLinea))) = 'ENTREGADO'
...
def _bodega_dashboard_estado_passes_filter(
    ...
    if not tf or tf in ('en_ruta', 'entregado', 'entrega_parcial_faena'):
        return True
    try:
    ...
    if request.method == 'POST':
        try:
            ...
            try:
                ...
                conn.commit()
                logger.info(f"Nuevo usuario registrado: {usuario} (ID: {new_user_id})")
                flash('Registro exitoso. ¡Ahora puedes iniciar sesión!', 'success')
                return redirect(url_for('frontend.login'))
                
            except Exception as e:
                conn.rollback()
                logger.error(f"Error en registro BD: {str(e)}", exc_info=True)
    ...
    try:
        ...
        try:
            ...
            if faena_mode:
                ...
                allowed_base_faena = set(faena_assigned_folio_set)
                if faena_cc_asignados:
                    allowed_base_faena |= _get_folios_by_centros_costo(
                        list(dict.fromkeys(faena_cc_asignados))
                    )

                cc_filter_active = bool((cc_filter_raw or "").strip())
                faena_folios_for_visibility = set()
                num_oc_only = int(num_oc_filtro_raw) if num_oc_filtro_raw.isdigit() else None
                selected_cc_mode = False
                cc_exact_erp_faena = None
                if cc_filter_active:
                    selected_cc_mode = True
                    cc_exact_erp_faena = _get_folios_softland_oc_cc_display_exact(cc_filter_raw)
                    cc_scoped = cc_exact_erp_faena & allowed_base_faena
                    if num_oc_only is not None:
                        if num_oc_only in cc_scoped:
                            faena_folios_for_visibility = {num_oc_only}
                    else:
                        faena_folios_for_visibility = set(cc_scoped)
                else:
                    effective_cc_tokens = (
                        list(dict.fromkeys(faena_cc_asignados)) if faena_cc_asignados else []
                    )
                    if num_oc_only is not None:
                ...
                if selected_cc_mode and not faena_folios_for_visibility:
                    faena_visibility_clause = "1=0"
                    faena_visibility_params = []
                elif faena_folios_for_visibility:
                    ...
                    if selected_cc_mode:
                        ...
                        """
                        faena_visibility_params = [folios_csv]
                    else:
                        faena_visibility_clause = """
                            (
                        ...
                        """
                        faena_visibility_params = [user_id, folios_csv]
                ...
                if tracking_estado_raw == 'recepcion_parcial':
                    try:
                        ...
                        if cc_filter_active and cc_exact_erp_faena is not None:
                            _rp = {
                                x
                                for x in _rp
                                if int(x) in cc_exact_erp_faena and int(x) in allowed_base_faena
                            }
                ...
                if cc_filter_raw:
                    ordenes = [o for o in ordenes if _orden_row_matches_cc_dashboard(o, cc_filter_raw)]
            ...
            try:
                ...
                if cached:
                    ...
                    conn_softland = pyodbc.connect(SoftlandConfig.get_connection_string(), timeout=10)
                    cursor_s = conn_softland.cursor()

                    if cc_filter_raw:
                        if tracking_estado_raw == 'entrega_parcial_faena' and folios_envio_parcial:
                            folios_envio_parcial = _filter_folio_ints_by_softland_cc_dashboard(
                                cursor_s, folios_envio_parcial, cc_filter_raw
                            )
                        elif tracking_estado_raw == 'en_ruta' and folios_en_ruta_list:
                            folios_en_ruta_list = _filter_folio_ints_by_softland_cc_dashboard(
                                cursor_s, folios_en_ruta_list, cc_filter_raw
                            )
                        elif tracking_estado_raw == 'entregado' and folios_entregados_list:
                            folios_entregados_list = _filter_folio_ints_by_softland_cc_dashboard(
                                cursor_s, folios_entregados_list, cc_filter_raw
                            )
                        if tracking_estado_raw == 'entrega_parcial_faena':
                            folios_envio_parcial_set = frozenset(folios_envio_parcial)
                    ...
                    if tracking_estado_raw == 'entrega_parcial_faena':
                        ...
                        if not has_any_role(user_role, ['FAENA']):
                            ...
                            _append_softland_oc_cc_dashboard_clause(where_eb_parts, where_eb_params, cc_filter_raw)
                        ...
                        _append_softland_oc_cc_dashboard_clause(where_softland_parts, where_params, cc_filter_raw)
                        ...
                        loc_list = []
                        for x in (folios_recepcion_parcial_local or []):
                            try:
                                loc_list.append(int(x))
                            except (TypeError, ValueError):
                                continue
                        if cc_filter_raw and loc_list:
                            loc_list = _filter_folio_ints_by_softland_cc_dashboard(
                                cursor_s, loc_list, cc_filter_raw
                            )
                        loc_rp = set(loc_list)
                ...
                logger.error(f"Softland no disponible en dashboard: {e}")
            ...
            local_stats_params = []
            if filtro_desde_date:
                local_stats_where_extra += " AND CAST(COALESCE(FechaHoraEntrega, FechaHoraSalida) AS date) >= ?"
            ...
            if has_any_role(user_role, ['FAENA']):
                cursor.execute(f"""
                    ...
                    WHERE transportista_asignado_id = ?
                    {local_stats_where_extra}
                """, tuple([user_id] + local_stats_params))
            else:
                cursor.execute(f"""
                    SELECT
                        COUNT(1) AS TotalOC,
            ...
            if not master_data:
                ...
                if skip_unfiltered_tracking_fallback:
                    ...
                    if tracking_estado_raw == 'en_ruta':
                        ...
                        if filtro_desde_date:
                            fallback_sql += (
                                " AND (U.FechaHoraSalida IS NULL OR CAST(U.FechaHoraSalida AS date) >= ?)"
                            )
                        ...
                        if filtro_desde_date:
                            fallback_sql += (
                                ...
                                "OR CAST(COALESCE(U.FechaHoraEntrega, U.FechaHoraSalida) AS date) >= ?)"
                            )
                            fallback_params.append(filtro_desde_raw)
                        if filtro_hasta_date:
                            fallback_sql += (
                        ...
                        if filtro_desde_date:
                            fallback_sql += (
                                " AND CAST(COALESCE(FechaHoraEntrega, FechaHoraSalida) AS date) >= ?"
                            )
                            fallback_params.append(filtro_desde_raw)
                        if filtro_hasta_date:
                            fallback_sql += (
                    ...
                    fallback_sql += " ORDER BY NumOc DESC OFFSET ? ROWS FETCH NEXT ? ROWS ONLY"
                    fallback_params.extend([row_offset, page_size + 1])
            ...
            can_view_details = has_any_role(user_role, ['SUPERADMIN', 'VISUALIZADOR', 'BODEGA'])
            can_receive = has_any_role(user_role, ['FAENA'])
            can_reset_local_tracking = has_any_role(user_role, ['SUPERADMIN'])
            can_search_requisicion = has_any_role(user_role, ['SUPERADMIN', 'BODEGA', 'VISUALIZADOR', 'FAENA'])
...
def reset_local_tracking_data():
    ...
    try:
        with local_db_transaction() as (conn, cursor):
            _ensure_local_tracking_table(cursor, conn)
            cursor.execute("""
                IF OBJECT_ID('DespachosEnvioDetalle', 'U') IS NOT NULL DELETE FROM DespachosEnvioDetalle;
                IF OBJECT_ID('DespachosEnvio', 'U') IS NOT NULL DELETE FROM DespachosEnvio;
                IF OBJECT_ID('DespachosTrackingDetalle', 'U') IS NOT NULL DELETE FROM DespachosTrackingDetalle;
                IF OBJECT_ID('DespachosTracking', 'U') IS NOT NULL DELETE FROM DespachosTracking;
            """)
            cursor.execute("""
                IF OBJECT_ID('DespachosEnvioDetalle', 'U') IS NOT NULL DBCC CHECKIDENT ('DespachosEnvioDetalle', RESEED, 0);
                IF OBJECT_ID('DespachosEnvio', 'U') IS NOT NULL DBCC CHECKIDENT ('DespachosEnvio', RESEED, 0);
                IF OBJECT_ID('DespachosTrackingDetalle', 'U') IS NOT NULL DBCC CHECKIDENT ('DespachosTrackingDetalle', RESEED, 0);
                IF OBJECT_ID('DespachosTracking', 'U') IS NOT NULL DBCC CHECKIDENT ('DespachosTracking', RESEED, 0);
            """)
...
@login_required(roles=['SUPERADMIN', 'BODEGA', 'VISUALIZADOR', 'FAENA'])
...
@login_required(roles=['SUPERADMIN', 'BODEGA'])
def importar_oc():
    ...
    try:
        ...
        logger.error(f"Error importando OC {folio}: {str(e)}")
        flash('Error interno al importar la OC', 'danger')
        return redirect(url_for('frontend.index'))

# ============================================
# RUTAS PARA BODEGA (MEJORADAS)
# ============================================
...
@login_required(roles=['SUPERADMIN', 'VISUALIZADOR', 'BODEGA', 'FAENA'])
def recepcion_bodega(folio):
    ...
    try:
        ...
        if has_any_role(user_role, ['FAENA']) and not has_any_role(user_role, ['SUPERADMIN']):
            ...
            if not cc_allowed:
                ...
                try:
                    ...
                    conn_loc_chk.close()
                line_cc_ok = _faena_softland_items_match_assigned_cc(despacho_items, faena_cc)
                if not assigned_me and not partial_ok and not line_cc_ok:
                    logger.warning(
                        "FAENA %s sin acceso a OC %s (CC cabecera / líneas ERP / asignación / parcial)",
                        user_id,
                        folio,
                    )
                    abort(403)
        ...
        logger.error(f"Error crítico en la vista detalle OC {folio}: {str(e)}", exc_info=True)
...
def despacho_bodega(folio):
    ...
    try:
        if request.method == 'POST':
            ...
            def _post_phase1_validate(cursor, conn):
                ...
                if not tracking_current:
                    ...
                    tracking_current[0],
                    ('EN_BODEGA', 'INGRESADO', 'DISPONIBLE EN BODEGA', 'En Ruta'),
                ):
                ...
                for raw_idx in selected_indices:
                    ...
                    try:
                        qty_solicitada = float((request.form.get(f'item_qtysol_{idx}') or '0').replace(',', '.'))
                    except Exception:
                        qty_solicitada = 0.0
                    try:
                        qty_ingresada = float((request.form.get(f'item_qtying_{idx}') or '0').replace(',', '.'))
                    except Exception:
                        qty_ingresada = 0.0
                    if qty_ingresada <= 0:
                    ...
                    already_sent = (
                        float(sent_map_line.get(line_norm, 0.0))
                        if line_norm is not None
                        else float(sent_map.get(line_key, 0.0))
                    )
                    if already_sent + qty_send > (qty_ingresada + 1e-9):
                        raise _DespachoFlowExit(
                            'frontend.despacho_bodega',
                            (
                    ...
                    if qty_solicitada > 0 and already_sent + qty_send > (qty_solicitada + 1e-9):
                        raise _DespachoFlowExit(
                            'frontend.despacho_bodega',
                            (
            ...
            def _post_phase2_write(cursor, conn):
                nonlocal envio_id
                _ensure_local_tracking_table(cursor, conn)
                sent_map2 = _sum_enviado_por_linea(cursor, folio)
                sent_map_line2 = _sum_enviado_por_numlinea(cursor, folio)
                for item in selected_items:
                    num_linea = item[1]
                    codprod = item[2]
                    qty_ingresada = float(item[5] or 0)
                    qty_send = float(item[6] or 0)
                    qty_solicitada = float(item[4] or 0)
                    line_key = _bodega_envio_line_key(num_linea, codprod)
                    line_norm = _normalize_oc_linea_num(num_linea)
                    already_sent = (
                        float(sent_map_line2.get(line_norm, 0.0))
                        if line_norm is not None
                        else float(sent_map2.get(line_key, 0.0))
                    )
                    if already_sent + qty_send > (qty_ingresada + 1e-9):
                        raise _DespachoFlowExit(
                            'frontend.despacho_bodega',
                ...
                for item in selected_items:
                    ...
                    pend_line = max(qty_ing - prev_line, 0.0)
                    line_state = (
                        'PARCIAL'
                        if (qty_send + 1e-9 < qty_sol or qty_send + 1e-9 < pend_line)
                        else 'EN_RUTA'
                    )
                    if line_state == 'PARCIAL':
            ...
            try:
                ...
                logger.error(f"Error en despacho (fase escritura): {str(e)}", exc_info=True)
                flash(f'Error al despachar: {str(e)}', 'danger')
                return redirect(url_for('frontend.index'))
        ...
        logger.error(f"Error en despacho_bodega: {str(e)}", exc_info=True)
...
# ============================================
# VISTAS FAENA: OC POR CENTRO DE COSTO Y REQUISICIONES
# ============================================

@bp.route('/faena/ordenes')
@login_required(roles=['SUPERADMIN', 'FAENA'])
def faena_ordenes_cc():
    ...
    conn_softland = None
    try:
        conn_softland = pyodbc.connect(SoftlandConfig.get_connection_string(), timeout=15)
        cursor_s = conn_softland.cursor()
        cursor_s.execute(
            f"""
            SELECT COUNT(1)
...
def mis_entregas():
    ...
    try:
        ...
        logger.error(f"Error en mis_entregas: {str(e)}", exc_info=True)
...
def recibir_producto(envio_id):
    ...
    try:
        ...
        try:
            ...
            if request.method == 'POST':
                ...
                try:
                    ...
                    if not _state_in(envio_row[2], ('En Ruta',)):
                    ...
                    if detail_rows:
                        for row in detail_rows:
                            det_id = int(row[0])
                            qty_env = float(row[4] or 0)
                            field = f'qty_{det_id}'
                            qrec = _parse_qty_faena_recepcion_linea(
                                request.form.get(field),
                                qty_env,
                            )
                            if qrec is None:
                                flash(
                                    f'Cantidad recibida inválida para '
                                    f'{(row[2] or "N/A")} (línea {row[1] if row[1] is not None else "-"}): '
                                    f'indique un valor entre 0 y {qty_env:.4f}.',
                                    'warning',
                                )
                                return redirect(receive_form_url)
                            line_qty_by_id[det_id] = qrec
                        total_rec = sum(line_qty_by_id.values())
                        if total_rec <= 1e-9:
                            flash(
                                'Debe indicar al menos una cantidad recibida mayor a cero '
                                '(o use 0 en cada línea si no corresponde marcar esta guía como recibida).',
                                'warning',
                            )
                            return redirect(receive_form_url)
                    ...
                    if detail_rows:
                        for row in detail_rows:
                            det_id = int(row[0])
                            qty_env = float(row[4] or 0)
                            qty_rec = line_qty_by_id.get(det_id, 0.0)
                            if qty_rec <= 1e-9:
                                cursor.execute(
                                    """
                                    UPDATE DespachosEnvioDetalle
                                    SET EstadoLinea = 'EN_RUTA',
                                        CantidadRecibidaFaena = NULL,
                                        FechaRecepcion = NULL,
                                        RecibidoPor = NULL
                                    WHERE Id = ?
                                      AND EnvioId = ?
                            ...
                            elif abs(qty_rec - qty_env) <= 1e-5:
                                cursor.execute(
                                    """
                                    UPDATE DespachosEnvioDetalle
                                    SET EstadoLinea = 'ENTREGADO',
                                        CantidadRecibidaFaena = NULL,
                                ...
                                cursor.execute(
                                    ...
                                    UPDATE DespachosEnvioDetalle
                                    SET EstadoLinea = 'ENTREGADO',
                                        CantidadRecibidaFaena = ?,
                                        FechaRecepcion = GETDATE(),
                                        RecibidoPor = ?
                                    WHERE Id = ?
                                      AND EnvioId = ?
                                    """,
                                    (qty_rec, user_id, det_id, envio_id),
                                )
                    ...
                    logger.info(f"Entrega registrada: Envío {envio_id}, OC {folio}, Usuario {user_id}")
            ...
            if not _state_in(envio_row[2], ('En Ruta', 'Entregado')):
        ...
        logger.error(f"Error en recibir_producto: {str(e)}", exc_info=True)
        flash('Error inesperado', 'danger')
        return redirect(url_for('frontend.index'))
...
# ============================================
# API PARA VERIFICACIÓN QR (app móvil)
# ============================================

@bp.route('/api/verificar_qr', methods=['POST'])
@login_required(roles=['SUPERADMIN', 'BODEGA', 'FAENA'])
def verificar_qr():
...
def gestionar_usuarios():
    ...
    roles_creables = ['SUPERADMIN', 'BODEGA', 'VISUALIZADOR', 'FAENA']
...
@login_required(roles=['VISUALIZADOR'])
...
def forbidden(error):
    logger.warning(f"403 Forbidden: {request.path} - Usuario: {session.get('user_id')}")
