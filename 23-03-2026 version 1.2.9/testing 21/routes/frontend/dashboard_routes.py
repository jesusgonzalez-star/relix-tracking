...
def debug_rechazados():
    ...
    try:
        ...
        if folios:
            ph = ','.join(['?']*len(folios))
            cursor.execute(f"SELECT NumOc, Estado, transportista_asignado_id FROM DespachosTracking WHERE NumOc IN ({ph})", tuple(folios))
            trk = cursor.fetchall()
...
def supervisor_contratos():
    ...
    try:
        ...
        # Query OPTIMIZADA: obtener todo de una sola pasada
        query = """
        ...
        # Procesar datos en memoria (más rápido que múltiples queries)
        requisiciones_dict = {}
        ordenes_dict = {}

        for row in all_rows:
            # Índices: 0=NumReq, 1=FEmision, 2=FReq, 3=CodEstado, 4=Solicitante, 5=CentroCosto,
            #          6=NumOC, 7=FechaOC, 8=Proveedor, 9=Monto, 10=CodProd, 11=DescProd, 12=Cantidad
            num_req = row[0]
            num_oc = row[6]
            cod_prod = row[10]
            ...
            if num_req not in requisiciones_dict:
                # Convertir fechas correctamente (pueden ser datetime o string)
                fecha_emision = row[1]
            ...
            if num_oc and num_oc not in ordenes_dict:
                # Convertir fecha OC correctamente
                fecha_oc = row[7]
        ...
        # Obtener tracking local de una sola pasada (solo BD local, sin referencias a Softland)
        cursor_l.execute("""
        ...
        for tr_row in tracking_rows:
            tracking_map[tr_row[0]] = [
                tr_row[1],
                tr_row[2].isoformat() if tr_row[2] else None,
                tr_row[3],
                None
            ]

        # Agregar tracking a ordenes
