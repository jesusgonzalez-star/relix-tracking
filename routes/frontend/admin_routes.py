...
from routes.frontend._helpers import (
    ...
    _invalidate_softland_dashboard_cache,
    logger,
)
...
def gestionar_usuarios():
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            try:
                if action == 'delete':
                    ...
                    # Descubre FKs que apuntan a UsuariosSistema.
                    ...
                    cursor.execute(
                        ...
                        WHERE TABLE_SCHEMA = DATABASE()
                          AND REFERENCED_TABLE_NAME = 'UsuariosSistema'
                          AND REFERENCED_COLUMN_NAME = 'Id'
                        """
                    )
                    fk_columns = [(r[0], r[1]) for r in cursor.fetchall()]
                    ...
                    for table_name, column_name in fk_columns:
                        ...
                        if not column_name.isidentifier():
                            ...
                            continue
                        cursor.execute(
                            f"UPDATE {quote_ident(table_name)} SET {quote_ident(column_name)} = ? "
                            f"WHERE {quote_ident(column_name)} = ?",
                            (actor_id, usuario_id),
                        )
                    ...
                    audit_log(
                        ...
                        detalles=f'Usuario eliminado: {target[0]} (rol: {target[1]})'
                    )
                ...
                if action == 'change_password':
                    ...
                    conn.commit()
                    # Auditoría: registro de cambio de contraseña
                    audit_log(
                        tabla='UsuariosSistema',
                        accion='UPDATE',
                        usuario=session.get('user_id'),
                        registro_id=usuario_id,
                        detalles=f'Contraseña cambio por SUPERADMIN para usuario {target[1]}'
                    )
                ...
                if action == 'update_cc':
                    ...
                    rol_target_name = (rol_target[0] or '').upper() if rol_target else ''
                    if rol_target_name not in ('FAENA', 'SUPERVISOR_CONTRATO'):
                        flash('Los centros de costo solo se asignan a usuarios FAENA o SUPERVISOR_CONTRATO.', 'warning')
                        return redirect(url_for('frontend.gestionar_usuarios'))
                    cursor.execute("""
                        UPDATE UsuariosSistema
                    ...
                    conn.commit()
                    # Auditoría: registro de cambio de centros de costo
                    audit_log(
                        tabla='UsuariosSistema',
                        accion='UPDATE',
                        usuario=session.get('user_id'),
                        registro_id=usuario_id,
                        detalles=f'Centros de costo actualizados: {cc_to_save}'
                    )
        ...
        cc_por_usuario = {}
        for u in usuarios:
            if len(u) > 6 and (u[4] or '').upper() in ('FAENA', 'SUPERVISOR_CONTRATO'):
                cc_por_usuario[u[0]] = _normalize_cc_assignments(u[6] or '')
    ...
    try:
        ...
        try:
            ...
            if _erp_scopes_softland_by_aux(user_role) and aux_id_softland:
                where_softland += " AND NumInterOc IN (SELECT NumInterOC FROM softland.owordencom WITH (NOLOCK) WHERE CodAux = ?)"
...
def reportes():
    ...
    try:
        ...
        try:
            ...
            # Paso 1: Obtener OCs del último mes y sus guías desde Softland
            softland_oc_data = {}  # {NumOc: tiene_guia}
            try:
                ...
                try:
                    ...
                    cursor_s.execute("""
                        ...
                        LEFT JOIN softland.IW_vsnpGuiasEntradaxOC G WITH (NOLOCK)
                            ON OC.NumOc = G.Orden
                        WHERE COALESCE(
                            TRY_CONVERT(date, OC.FechaOC, 103),
                            TRY_CONVERT(date, OC.FechaOC)
                        ) >= DATEADD(month, -1, GETDATE())
                        GROUP BY OC.NumOc
                    """)
                    for row in cursor_s.fetchall():
                        softland_oc_data[int(row[0])] = (row[1] is not None)
                finally:
                    conn_softland.close()
            except Exception as exc:
                logger.warning("Softland no disponible para reportes: %s", exc)

            # Paso 2: Obtener estado local de tracking para esas OCs
            local_estado = {}  # {NumOc: estado}
            if softland_oc_data:
                folios = list(softland_oc_data.keys())
                # Construir SQL sin f-strings para evitar SQL injection
                placeholders_str = ",".join(["?"] * len(folios))
                sql = """
                    SELECT NumOc, Estado
                    FROM DespachosTracking
                    WHERE NumOc IN (""" + placeholders_str + ")"
                cursor.execute(sql, tuple(folios))
                for row in cursor.fetchall():
                    num_oc = int(row[0])
                    estado = (row[1] or '').strip()
                    if estado:
                        local_estado[num_oc] = estado

            # Paso 3: Calcular estadísticas en Python
            total, en_bodega, despachados, entregados, pendientes = 0, 0, 0, 0, 0
            for num_oc, tiene_guia in softland_oc_data.items():
                total += 1
                estado_raw = local_estado.get(num_oc)
                if estado_raw:
                    est_norm = estado_raw.replace('_', ' ').strip().upper()
                elif tiene_guia:
                    est_norm = 'EN BODEGA'
                else:
                    est_norm = 'PENDIENTE_EN_SOFTLAND'

                if est_norm in ('EN BODEGA', 'INGRESADO', 'DISPONIBLE EN BODEGA'):
                    en_bodega += 1
                elif est_norm == 'EN RUTA':
                    despachados += 1
                elif est_norm == 'ENTREGADO':
                    entregados += 1
                elif est_norm == 'PENDIENTE_EN_SOFTLAND':
                    pendientes += 1

            stats = (total, en_bodega, despachados, entregados, pendientes)

            # Rendimiento por usuario (BD local MariaDB)
            cursor.execute("""
                SELECT
                    U.NombreCompleto,
                ...
                FROM DespachosTracking D
                LEFT JOIN UsuariosSistema U ON D.RegistradoPor = U.Id
                WHERE D.FechaHoraSalida >= DATE_SUB(NOW(), INTERVAL 1 MONTH)
...
@bp.route('/superadmin/invalidar-cache', methods=['POST'])
@login_required(roles=roles_for('manage_users'))
def invalidar_cache_dashboard():
    """Invalida caché de dashboard (fuerza refresh desde ERP)."""
    try:
        _invalidate_softland_dashboard_cache()
        flash('Caché de dashboard invalidado. Los datos se actualizarán en la próxima consulta.', 'success')
    except Exception as e:
        logger.error(f"Error invalidando caché: {str(e)}", exc_info=True)
        flash('Error al invalidar caché', 'danger')
    return redirect(url_for('frontend.index'))
