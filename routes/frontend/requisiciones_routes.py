...
def detalle_requisicion(folio):
    ...
    try:
        ...
        is_super_rq = has_any_role(user_role_rq, ['SUPERADMIN'])
        user_cc_rq = None

        if not is_super_rq and has_any_role(user_role_rq, ['FAENA', 'SUPERVISOR_CONTRATO']):
            user_cc_rq = _fetch_faena_cc_for_user(user_id_rq)
            if not user_cc_rq:
                flash('No tiene centros de costo asignados para consultar requisiciones.', 'warning')
                return redirect(url_for('frontend.index'))
            cc_match_sql_rq, _ = _build_softland_cc_match_clause('OC', len(user_cc_rq))
            cursor.execute(
                f"""
                ...
                WHERE OC.NumOc = ?
                  AND {cc_match_sql_rq}
                """,
                tuple([folio] + list(user_cc_rq) + list(user_cc_rq)),
            )
            if not cursor.fetchone():
                logger.warning('Usuario %s sin acceso a requisición de OC %s (CC)', user_id_rq, folio)
                abort(403)
        ...
        # Validar que las columnas resueltas son seguras antes de interpolar
        _SAFE_CC_COLS = {'CodiCC', 'CodCC', 'CentroCosto'}
        if q_cc_col and q_cc_col not in _SAFE_CC_COLS:
            logger.error(f"Columna CodiCC no segura en owreqoc: {q_cc_col}")
            q_cc_col = None
        if o_cc_col and o_cc_col not in _SAFE_CC_COLS:
            logger.error(f"Columna CodiCC no segura en owordencom: {o_cc_col}")
            o_cc_col = None
...
def buscar_requisicion():
    """Busca una requisición y devuelve las OCs relacionadas (con paginación)."""
    ...
    if not num_req:
        return render_template('requisicion_busqueda.html', num_req='', resultados=[], modo='contiene', page=1, has_more=False, total_count=0)
    ...
    try:
        ...
        if not is_super:
            supervisor_cc = _fetch_faena_cc_for_user(user_id)
            if supervisor_cc:
                cc_match_sql, _ = _build_softland_cc_match_clause('O', len(supervisor_cc))
        ...
        count_query = f"""
            ...
            FROM softland.owreqoc Q
            JOIN softland.owordencom O ON Q.NumInterOC = O.NumInterOC
            WHERE {filtro_sql}{where_clause_cc}
        """
        count_params = [filtro_param] + query_params
    ...
    return render_template('requisicion_busqueda.html', num_req=num_req, resultados=resultados, modo=modo, page=page, has_more=has_more, total_count=total_count)
