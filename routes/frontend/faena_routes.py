...
def mis_entregas():
    ...
    try:
        ...
        try:
            ...
            params_local = []
            if desde:
                query_local += " AND DATE(COALESCE(E.FechaHoraEntrega, E.FechaHoraSalida)) >= ?"
        ...
        try:
            ...
            if request.method == 'POST':
                ...
                try:
                    ...
                    # Validación robusta de imagen (magic bytes, dimensiones, tamaño)
                    foto.seek(0)
                    is_valid_img, msg_img = validate_image_file(foto)
                    if not is_valid_img:
                        flash(f'Validación de imagen fallida: {msg_img}', 'warning')
                        return redirect(receive_form_url)
                    ...
                    for row in detail_rows:
                        ...
                        try:
                            ...
                            if qty_recibida > qty_enviada + _TOL:
                                flash(
                                    f'Cantidad recibida ({qty_recibida}) no puede superar la '
                                    f'enviada ({qty_enviada}) para el producto {cod_prod} '
                                    f'(línea {num_linea or "-"}). Si llegó material extra, '
                                    'contacte a bodega para regularizar.',
                                    'warning',
                                )
                                return redirect(receive_form_url)
                        except Exception:
                    ...
                    cursor.execute("""
                        UPDATE DespachosEnvio SET
                            Estado = ?,
                            FechaHoraEntrega = UTC_TIMESTAMP(),
                            ...
                            Observaciones = CASE
                                WHEN ? IS NOT NULL AND LENGTH(?) > 0 THEN
                                    ...
                                    || 'Recepción Faena: '
                                    || ?
                                    || CASE WHEN ? IS NOT NULL AND LENGTH(?) > 0 THEN ' | Geo:' || ? ELSE '' END
                                WHEN ? IS NOT NULL AND LENGTH(?) > 0 THEN
                                    COALESCE(Observaciones, '') || CASE WHEN Observaciones IS NULL OR Observaciones = '' THEN '' ELSE ' | ' END || 'Geo:' || ?
                                ELSE Observaciones
                            END
                        WHERE Id = ?
                          AND UPPER(LTRIM(RTRIM(REPLACE(Estado, '_', ' ')))) = 'EN RUTA'
                    ...
                    # Auditoría: registro de recepción de envío
                    audit_log(
                        tabla='DespachosEnvio',
                        accion='UPDATE',
                        usuario=session.get('user_id'),
                        registro_id=envio_id,
                        detalles=f'Envío OC {folio} recepcionado en FAENA'
                    )
