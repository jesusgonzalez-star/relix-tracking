...
def importar_oc():
    ...
    try:
        with local_db_transaction() as (conn, cursor):
            ...
            # Lectura con orden determinista para evitar leer una fila intermedia
            # cuando DespachosTracking acumula historial append-only.
            cursor.execute(
                "SELECT Estado FROM DespachosTracking WHERE NumOc = ? "
            ...
            try:
                ...
                logger.warning(
                    'OC %s importada en modo LOCAL_SIN_ERP (Softland no accesible). '
                    'Despacho quedará bloqueado hasta que Softland reporte llegadas.',
                    folio,
                )
                flash(
                    f'OC #{folio} creada en tracking local (modo sin ERP). '
                    'El despacho se habilitará cuando Softland registre la llegada.',
                    'warning',
                )
            return redirect(url_for('frontend.index'))
    except RuntimeError:
...
def recepcion_bodega(folio):
    ...
    try:
        ...
        if (has_any_role(user_role, ['FAENA', 'SUPERVISOR_CONTRATO']) and
            not has_any_role(user_role, ['SUPERADMIN'])):
            user_cc = _fetch_faena_cc_for_user(user_id)
            if not user_cc:
            ...
            try:
                cc_c = conn_cc.cursor()
                cc_match_sql, _ = _build_softland_cc_match_clause('OC', len(user_cc))
                cc_c.execute(
                    f"""
                    ...
                    WHERE OC.NumOc = ?
                      AND {cc_match_sql}
                    """,
                    tuple([folio] + list(user_cc) + list(user_cc)),
                )
                cc_allowed = bool(cc_c.fetchone())
            finally:
                conn_cc.close()
            if not cc_allowed:
                # Para FAENA: permitir si la guía está asignada al usuario o la OC figura como
                # recepción parcial en BD local.
                if has_any_role(user_role, ['FAENA']):
                    ...
                    # Para SUPERVISOR_CONTRATO: denegar acceso si no está en el CC
                    logger.warning("SUPERVISOR_CONTRATO %s sin acceso a OC %s (CC)", user_id, folio)
                    abort(403)
        ...
        if conn_local:
            try:
                ...
                if despacho_items:
                    ...
                    conn_local.commit()
                c_local.execute("""
                    SELECT NumOc, Estado, GuiaDespacho, UrlFotoEvidencia, FechaHoraSalida, FechaHoraEntrega
                    FROM DespachosTracking
                    WHERE NumOc = ?
                    ORDER BY Id DESC
                    LIMIT 1
                ...
                try:
                    c_local.execute("""
                        SELECT
                            ...
                            EstadoLinea,
                            MotivoRechazo,
                            CAST(CantidadRecibida AS DOUBLE) AS QtyRecibida
                ...
                if not detalle_local:
                    c_local.execute("""
                        SELECT
                            ...
                            CAST(CantidadEnviada AS DOUBLE) AS QtyRecibida
    ...
    try:
        if request.method == 'POST':
            ...
            def _post_phase1_validate(cursor, conn):
                _ensure_local_tracking_table(cursor, conn)
                cursor.execute(
                    "SELECT Estado FROM DespachosTracking WHERE NumOc = ? ORDER BY Id DESC LIMIT 1",
                    (folio,),
                )
        ...
        with local_db_transaction() as (conn, cursor):
            _ensure_local_tracking_table(cursor, conn)
            cursor.execute(
                "SELECT NumOc, Estado FROM DespachosTracking WHERE NumOc = ? ORDER BY Id DESC LIMIT 1",
                (folio,),
            )
