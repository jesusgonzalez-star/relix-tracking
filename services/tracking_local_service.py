...
from utils.states import API_TO_DB_ESTADO, is_valid_transition
...
def create_tracking_row(
    *,
    ...
    # Idempotency check con SELECT ... FOR UPDATE: evita que dos hilos pasen
    # ambos el check y terminen en IntegrityError.
    if key:
        existing = (
            DespachoTracking.query.filter_by(api_idempotency_key=key)
            .with_for_update()
            .first()
        )
    ...
    last = (
        ...
        .order_by(DespachoTracking.created_at.desc(), DespachoTracking.id.desc())
        .with_for_update()
        .first()
    )
    current_estado = last.estado if last is not None else None
    if not is_valid_transition(current_estado, db_estado):
        db.session.rollback()
        raise ValueError(
            f"Transición de estado inválida para OC {num_oc}: "
            f"{current_estado!r} → {db_estado!r}"
        )
