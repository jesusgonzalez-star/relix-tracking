...
def ensure_faena_cc_column(cursor):
    """Asegura columna de centros de costo asignados por usuario (MariaDB)."""
    cursor.execute(
        "SELECT COLUMN_NAME FROM information_schema.columns "
        "WHERE table_schema = DATABASE() AND TABLE_NAME = 'UsuariosSistema'"
    )
    existing = {row[0] for row in cursor.fetchall()}
    if 'CentrosCostoAsignados' not in existing:
        cursor.execute("ALTER TABLE UsuariosSistema ADD COLUMN CentrosCostoAsignados TEXT")
...
def fetch_softland_centros_costo_opciones(usuarios_rows=None):
    ...
    if usuarios_rows:
        for u in usuarios_rows:
            if len(u) > 6 and (u[4] or '').upper() in ('FAENA', 'SUPERVISOR_CONTRATO') and u[6]:
