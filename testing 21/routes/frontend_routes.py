...
def _get_folios_by_centros_costo(cc_tokens):
    ...
    try:
        ...
        try:
            ...
            cc_expr = "UPPER(LTRIM(RTRIM('SIN CC')))"
            if cc_desc_col and cc_code_col:
                cc_expr = (
                    f"UPPER(LTRIM(RTRIM(COALESCE(NULLIF(OC.{cc_desc_col}, ''), NULLIF(OC.{cc_code_col}, ''), 'SIN CC'))))"
                )
            elif cc_desc_col:
                cc_expr = f"UPPER(LTRIM(RTRIM(COALESCE(NULLIF(OC.{cc_desc_col}, ''), 'SIN CC'))))"
            elif cc_code_col:
                cc_expr = f"UPPER(LTRIM(RTRIM(COALESCE(NULLIF(OC.{cc_code_col}, ''), 'SIN CC'))))"

            ph = ",".join(["?"] * len(cc_tokens))
            c.execute(
                f"""
                SELECT OC.NumOc
    ...
    try:
        ...
        try:
            ...
            cc_c.execute(
                ...
                WHERE OC.NumOc = ?
                  AND UPPER(LTRIM(RTRIM(COALESCE(NULLIF(OC.DescCC, ''), NULLIF(OC.CodiCC, ''), 'SIN CC')))) IN ({ph})
                """,
                tuple([folio] + list(cc_tokens)),
            )
            return cc_c.fetchone() is not None
        finally:
            conn_cc.close()
    except Exception as exc:
        logger.warning("No fue posible validar acceso FAENA por CC para folio %s: %s", folio, exc)
        return False
...
def _load_master_data_entrega_parcial_faena(
    ...
    for r in fetched:
        if r and r[0] is not None:
            by_f[int(r[0])] = r
    viz_aux = _erp_scopes_softland_by_aux(user_role) and aux_id_softland
...
def _load_reception_summary_by_folio(cursor, folios):
    """Resume recepción por OC (no/parcial/completa) basado en detalle local."""
    normalized = [int(f) for f in folios if f is not None]
    if not normalized:
        return {}
    placeholders = ",".join(["?"] * len(normalized))
    cursor.execute(
        ...
        SELECT
            X.NumOc,
            COUNT(1) AS TotalLineas,
            SUM(CASE WHEN UPPER(LTRIM(RTRIM(X.EstadoLinea))) = 'ENTREGADO' THEN 1 ELSE 0 END) AS LineasEntregadas
        FROM (
            SELECT D.NumOc, D.EstadoLinea
            FROM DespachosEnvioDetalle D
            WHERE D.NumOc IN ({placeholders})
            UNION ALL
            SELECT LD.NumOc, LD.EstadoLinea
            FROM DespachosTrackingDetalle LD
            WHERE LD.NumOc IN ({placeholders})
              AND NOT EXISTS (SELECT 1 FROM DespachosEnvioDetalle D2 WHERE D2.NumOc = LD.NumOc)
        ) AS X
    ...
    for row in cursor.fetchall():
        ...
        summary[folio] = {
            'total_lineas': total,
            'lineas_entregadas': delivered,
            'status_label': (
                'No recepcionado' if delivered == 0 else
                'Recepcionado parcial' if delivered < total else
                'Recepcionado completo'
            ) if total > 0 else 'No recepcionado'
        }
    return summary
    ...
    try:
        cursor.execute(
            ...
            SELECT
                ...
                SUM(CAST(D.CantidadEnviada AS DECIMAL(18, 4))) AS Q
            FROM DespachosEnvioDetalle D
            WHERE D.NumOc = ?
...
def _derive_bodega_tracking_status(total_solicitada, total_ingresada):
    """Calcula estado operativo por cantidad llegada vs solicitada."""
    try:
    ...
    if qty_ingresada <= 0:
        return 'No entregado'
    if qty_solicitada > 0 and qty_ingresada + 1e-9 < qty_solicitada:
        return 'Recepción parcial'
    return 'Entrega total'

def _matches_bodega_tracking_filter(status_label, tracking_filter):
    """Valida coincidencia entre estado visual y filtro solicitado."""
    ...
    expected_label = mapping.get((tracking_filter or '').strip().lower())
        ...
        try:
            ...
            if faena_mode:
                ...
                if faena_folios_for_visibility:
                    ...
                    """
                    faena_visibility_params.append(folios_csv)
                ...
                local_rows = local_rows[:page_size]
                partial_map = _load_partial_flags_by_folio(cursor, [row[0] for row in local_rows])
                reception_summary_map = _load_reception_summary_by_folio(cursor, [row[0] for row in local_rows])
                ...
                for trk in local_rows:
                    ...
                    eta_label, eta_class = _build_eta_badge(softland_row.get('fecha_eta'), estado_tracking, trk[3])
                    reception_status = reception_summary_map.get(folio, {}).get('status_label', 'No recepcionado')
                    has_any_arrival = reception_status in ('Recepcionado parcial', 'Recepcionado completo') or _state_in(estado_tracking, ('En Ruta', 'Entregado'))
            ...
            conn_softland = None
            use_softland_dashboard_cache = tracking_estado_raw not in ('entrega_parcial_faena', 'en_ruta')
            try:
                cache_key = (
            ...
            for row in master_data:
                ...
                reception_status = reception_summary_map.get(folio_int, {}).get('status_label', 'No recepcionado')
                bodega_tracking_status = _derive_bodega_tracking_status(qty_solicitada_total, qty_ingresada_total)
            ...
            if not master_data:
                ...
                for folio_local, trk in fallback_items:
                    ...
                    reception_status = reception_summary_map.get(int(folio_local), {}).get('status_label', 'No recepcionado')
                    has_any_arrival = (
                        reception_status in ('Recepcionado parcial', 'Recepcionado completo')
                        or _state_in(estado_tracking, ('En Ruta', 'Entregado'))
                    )
                    bodega_tracking_status = (
                        'Entrega total' if reception_status == 'Recepcionado completo' else
                        'Recepción parcial' if has_any_arrival else
    ...
    try:
        ...
        if has_any_role(user_role_rq, ['FAENA']) and not has_any_role(user_role_rq, ['SUPERADMIN']):
            ...
            if not faena_cc_rq:
                ...
                return redirect(url_for('frontend.index'))
            ph_rq = ','.join(['?'] * len(faena_cc_rq))
            cursor.execute(
                f"""
                SELECT 1 FROM softland.OW_vsnpTraeEncabezadoOCompra OC WITH (NOLOCK)
                WHERE OC.NumOc = ?
                  AND UPPER(LTRIM(RTRIM(COALESCE(NULLIF(OC.DescCC, ''), NULLIF(OC.CodiCC, ''), 'SIN CC')))) IN ({ph_rq})
                """,
                tuple([folio] + list(faena_cc_rq)),
            )
...
def importar_oc():
    ...
    try:
        ...
        # 1. Verificar si ya está en local
        ...
        # 2. Consultar SoftlandService
        from services.softland_service import SoftlandService
        from utils.errors import APIError
        try:
            detalle = SoftlandService.obtener_detalle_oc(folio)
            
            # Verificar si Softland indica que ya entró (opcional pero sugerido)
            if not detalle.get('guia_entrada'):
                flash('Atención: La OC existe en Softland pero no tiene Guía de Entrada en el ERP aún.', 'warning')
            
            # 3. Importar a Tabla local como INGRESADO
            cursor.execute("""
            ...
            flash(f'¡OC #{folio} importada con éxito! (Proveedor: {detalle.get("proveedor")}). Lista en Bodega.', 'success')
        except APIError as e:
            # Fallback local para operar tracking propio sin ERP disponible.
            cursor.execute("""
...
def _load_envios_agrupados_por_guia(cursor, num_oc):
    ...
    try:
        ...
        for ev in envios:
            ...
            cursor.execute(
                """
                SELECT NumLineaOc, CodProd, DescripcionProd, CantidadEnviada, EstadoLinea
            ...
            try:
                ...
                cc_c.execute(
                    ...
                    WHERE OC.NumOc = ?
                      AND UPPER(LTRIM(RTRIM(COALESCE(NULLIF(OC.DescCC, ''), NULLIF(OC.CodiCC, ''), 'SIN CC')))) IN ({ph})
                    """,
                    tuple([folio] + list(faena_cc)),
                )
                if not cc_c.fetchone():
                    logger.warning("FAENA %s sin acceso a OC %s (CC)", user_id, folio)
                    abort(403)
            finally:
                conn_cc.close()
        ...
        try:
            ...
            if request.method == 'POST':
                try:
                    ...
                    cursor.execute("SELECT TOP 1 Estado FROM DespachosTracking WHERE NumOc = ? ORDER BY Id DESC", (folio,))
                    ...
                    if not tracking_current:
                        ...
                        flash('La orden no está en un estado válido de bodega para despachar.', 'warning')
                        return redirect(url_for('frontend.index'))
                    ...
                    if items_present and not selected_indices:
                        flash('Debe seleccionar al menos un ítem para despachar.', 'warning')
                        return redirect(url_for('frontend.despacho_bodega', folio=folio))
                    ...
                    for raw_idx in selected_indices:
                        ...
                        if qty_send <= 0:
                            continue
                        # No usar sanitize_input aquí: borra ()'"= y rompe la clave vs. Softland / DespachosEnvioDetalle.
                        ...
                        if qty_solicitada > 0 and already_sent + qty_send > (qty_solicitada + 1e-9):
                            flash(
                                f'La cantidad acumulada a enviar para {codprod or idx} no puede superar la solicitada ({qty_solicitada}).',
                                'warning'
                            )
                            return redirect(url_for('frontend.despacho_bodega', folio=folio))
                    ...
                    if items_present and not selected_items:
                        flash('Los ítems seleccionados deben tener cantidad a enviar mayor a 0.', 'warning')
                        return redirect(url_for('frontend.despacho_bodega', folio=folio))
                    if not selected_items:
                        flash('Debe incluir al menos una línea con cantidad a enviar.', 'warning')
                        return redirect(url_for('frontend.despacho_bodega', folio=folio))

                    envio_parcial_flag = _compute_entrega_parcial_bodega_envio(
                        despacho_items_softland, sent_map, selected_items, sent_map_line
                    )
                    ...
                    if not receptor_row:
                        flash('El receptor asignado no es válido o no está activo.', 'danger')
                        return redirect(url_for('frontend.despacho_bodega', folio=folio))
                    ...
                    if not envio_id:
                        conn.rollback()
                        flash('No se pudo registrar el envío en base local.', 'danger')
                        return redirect(url_for('frontend.index'))
            ...
            # GET - Mostrar formulario basado en tracking local (modo robusto desde cero)
            cursor.execute("SELECT TOP 1 NumOc, Estado FROM DespachosTracking WHERE NumOc = ? ORDER BY Id DESC", (folio,))
            ...
            if not orden_local:
                if not arrival_summary['has_any_arrival']:
                    flash('Aún no puedes despachar esta OC: no hay productos llegados en bodega (ni parcial ni total).', 'warning')
                    return redirect(url_for('frontend.index'))
                cursor.execute("""
                    INSERT INTO DespachosTracking (NumOc, Estado, RegistradoPor, Observaciones)
                    VALUES (?, 'EN_BODEGA', ?, ?)
            ...
            if not _state_in(
                ...
                flash('La orden debe estar en estado de bodega (o con envíos en ruta) para despachar.', 'warning')
                return redirect(url_for('frontend.index'))
            if not arrival_summary['has_any_arrival']:
                flash('Aún no puedes despachar esta OC: no hay productos llegados en bodega (ni parcial ni total).', 'warning')
                return redirect(url_for('frontend.index'))
            ...
            if not receptores:
                flash('No hay perfiles FAENA activos para asignar este despacho.', 'warning')
                return redirect(url_for('frontend.index'))
...
def faena_ordenes_cc():
    ...
    if faena_cc is not None:
        ph = ','.join(['?'] * len(faena_cc))
        where_parts.append(
            f"UPPER(LTRIM(RTRIM(COALESCE(NULLIF(OC.DescCC, ''), NULLIF(OC.CodiCC, ''), 'SIN CC')))) IN ({ph})"
        )
        ...
        try:
            ...
            for oc in oc_rows:
                ...
                if trk:
                    ...
                    estado_tracking = 'PENDIENTE_EN_SOFTLAND'
                reception_status = reception_summary_map.get(folio, {}).get('status_label', 'No recepcionado')
                bodega_tracking_status = _derive_bodega_tracking_status(qty_sol, qty_ing)
                filas.append({
                    'folio': folio,
...
def faena_requisiciones():
    ...
    if faena_cc is not None:
        ph = ','.join(['?'] * len(faena_cc))
        where_parts.append(
            f"UPPER(LTRIM(RTRIM(COALESCE(NULLIF(O.DescCC, ''), NULLIF(O.CodiCC, ''), 'SIN CC')))) IN ({ph})"
        )
        params.extend(faena_cc)
    if num_req_raw:
    ...
    try:
        ...
        try:
            ...
            if has_any_role(user_role, ['FAENA']) and not has_any_role(user_role, ['SUPERADMIN']):
                ...
                if (not assigned_to_user) and (not _faena_user_has_cc_access_to_folio(user_id, folio)):
                    flash('No autorizado para este centro de costo.', 'danger')
                    return redirect(default_list_url)
                if not assigned_to_user:
                    flash('Este envío no está asignado a su perfil.', 'danger')
                    return redirect(default_list_url)
            ...
            if request.method == 'POST':
                ...
                try:
                    ...
                    if detail_rows:
                        expected_ids = {str(row[0]) for row in detail_rows}
                        confirmed_ids = set(request.form.getlist('item_received_id'))
                        if confirmed_ids != expected_ids:
                            missing = [
                                f"{(row[2] or 'N/A')} (línea {row[1] if row[1] is not None else '-'})"
                                for row in detail_rows if str(row[0]) not in confirmed_ids
                            ]
                            sample_missing = ", ".join(missing[:5])
                            extra = "..." if len(missing) > 5 else ""
                            flash(
                                f'Debe confirmar todos los productos de la guía antes de marcar recibido. '
                                f'Faltan: {sample_missing}{extra}',
                                'warning'
                            )
                            return redirect(receive_form_url)
                    ...
                    cursor.execute("""
                        ...
                        SET EstadoLinea = 'ENTREGADO',
                            ...
                            RecibidoPor = ?
                        WHERE EnvioId = ?
                          AND UPPER(LTRIM(RTRIM(EstadoLinea))) <> 'ENTREGADO'
                    """, (user_id, envio_id))
            ...
            for bucket in envios_oc_raw:
                ...
                for li in lineas:
                    try:
                        total_enviado += float(li[3] or 0.0)
                    except Exception:
                        continue
...
def recibir_producto_legacy_por_folio(folio):
    ...
    try:
        ...
        try:
            ...
            params = [folio]
            if has_any_role(user_role, ['FAENA']) and not has_any_role(user_role, ['SUPERADMIN']):
                has_cc_access = _faena_user_has_cc_access_to_folio(user_id, folio)
                if not has_cc_access:
                    cursor.execute("""
                        SELECT 1
                        FROM DespachosEnvio
                        WHERE NumOc = ? AND transportista_asignado_id = ?
                    """, (folio, user_id))
                    has_cc_access = cursor.fetchone() is not None
                if not has_cc_access:
                    flash('No autorizado para este centro de costo.', 'danger')
                    return redirect(url_for('frontend.index'))
